        ...
        <div className="bookdetails">
            ...
            <div className="bookdetails__container">
                <div className="bookdetails__book-cover-section book-cover-section">
                    ...
                    {volumeInfo.averageRating && (
                        <div className="book-cover-section__rating">
                            <span className="rating__stars">{renderRatingStars(volumeInfo.averageRating)}</span>
                ...
                <div className="bookdetails__book-info-section book-info-section">
                    ...
                    {volumeInfo.subtitle && <h2 className="title__subtitle">{volumeInfo.subtitle}</h2>}
                    
                    <div className="book-info-section__meta">
                        {volumeInfo.authors && <MetaItem label="Authors: " value={volumeInfo.authors.join(', ')} />}
                        {volumeInfo.publishedDate && <MetaItem label="Published: " value={volumeInfo.publishedDate} />}
                        {volumeInfo.publisher && <MetaItem label="Publisher: " value={volumeInfo.publisher} />}
                        {volumeInfo.pageCount && <MetaItem label="Pages: " value={volumeInfo.pageCount} />}
                        {volumeInfo.language && <MetaItem label="Language: " value={volumeInfo.language.toUpperCase()} />}
                        {volumeInfo.categories && <MetaItem label="Categories: " value={volumeInfo.categories.join(', ')} />}
                    </div>
                </div>
            </div>
            {volumeInfo.description && (
                        <div className="book-info-section__description">
                            <h3>Description</h3>
                            <p dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(volumeInfo.description) }} />
                        </div>
                    )}
        </div>
    );
